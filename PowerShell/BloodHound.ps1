                        ...
                        if($MemberPrimaryGroupName) {
                            ...
                            if ($PSCmdlet.ParameterSetName -eq 'CSVExport') {
                                ...
                                if ($ObjectType -eq 'user') {
                                    $Null = $Statements.Add( @{ "statement"="MERGE ($($ObjectType)1:$ObjectTypeCap { name: UPPER('$AccountName'), simplename: UPPER('$MemberName') }) MERGE (group2:Group { name: UPPER('$MemberPrimaryGroupName') }) MERGE ($($ObjectType)1)-[:MemberOf]->(group2)" } )
                                }
                                else {
